...
def sellTokens(kwargs,arenaSellAmount,logging):
    ...
    TokenToSellContract = BEP20Token(TokenToSellAddress,config.RPC_URL)
    ...
    if(not TokenToSellContract.checkTokenBalanceSufficientWithEther(config.YOUR_WALLET_ADDRESS,tokensToSellEther)):
        walletBalance = TokenToSellContract.getBalanceInWei(config.YOUR_WALLET_ADDRESS)/(10**18)
        raise Exception(f'Need {tokensToSellEther} CAKE to trade, but you only have {walletBalance} CAKE')
    ...
    # for testing
    if(config.DEBUGGING):
        [TokenSoldForUSDInEther,receipt] = getTokenBought(web3,"0x8264218ccc46f2d5be3f5d83d7d1a192ef69eadeb7943e54cc75f9d24d917ff2") # testing only
    ...
    if config.SELL_TOKENS:
        try:
            ...
            result = [web3.toHex(tx_token), f"Sold {web3.fromWei(tokenToSell, TradingTokenDecimal)} {symbol} for {TokenSoldForUSDInEther} CAKE  TransactionHash: {web3.toHex(tx_token)}"]
...
def getTokenBought(web3,txHash):
    ...
    for log in a:
        ...
        if(tokenContractAddress == TokenSoldForAddress): #get the amount of swapped token
            tokenSoldFor = value[0] / 10**tokenDecimal # shortcut, no token decimal parse from contract
            break
